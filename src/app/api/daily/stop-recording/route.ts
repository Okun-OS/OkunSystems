...
export async function POST(req: NextRequest) {
  try {
    ...
    if (apiKey && closingSession.dailyRecordingId && closingSession.appointment?.meetingUrl) {
      const roomName = closingSession.appointment.meetingUrl.split("/").pop();
      if (roomName) {
        const dailyRes = await fetch(
          `https://api.daily.co/v1/rooms/${roomName}/recordings/${closingSession.dailyRecordingId}`,
          {
            method: "DELETE",
        ...
        if (!dailyRes.ok) {
          ...
          console.warn("[stop-recording] Daily.co error:", err);
        }
      }
    }
    ...
    return NextResponse.json({ ok: true, recordingStatus: "stopped" });
  } catch (error) {
