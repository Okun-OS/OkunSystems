...
export default function NachrichtenPage() {
  ...
  return (
    <div className="max-w-[700px] mx-auto h-[calc(100vh-8rem)] flex flex-col">
      ...
      <div className="flex-1 bg-[#141414] border border-[#2a2a2a] rounded-xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center gap-3 p-4 border-b border-[#2a2a2a]">
          <div className="w-9 h-9 rounded-full bg-[#22c55e]/10 border border-[#22c55e]/20 flex items-center justify-center">
            <span className="text-[#22c55e] text-xs font-bold">FO</span>
          </div>
          <div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {MOCK_MESSAGES.map((msg) => (
            <div key={msg.id} className={`flex ${msg.from === "USER" ? "justify-end" : "justify-start"}`}>
              {msg.from === "OKUN" && (
                <div className="w-7 h-7 rounded-full bg-[#22c55e]/10 border border-[#22c55e]/20 flex-shrink-0 flex items-center justify-center mr-2.5 mt-auto">
                  <span className="text-[#22c55e] text-xs font-bold">FO</span>
                </div>
              )}
              <div className={`max-w-[80%] ${msg.from === "USER" ? "items-end" : "items-start"} flex flex-col`}>
                <div className={`px-4 py-2.5 rounded-xl text-sm ${
                  msg.from === "USER"
                    ? "bg-[#22c55e]/15 border border-[#22c55e]/25 text-[#f0f0f0] rounded-tr-sm"
                    : "bg-[#1a1a1a] border border-[#2a2a2a] text-[#f0f0f0] rounded-tl-sm"
                }`}>
                  {msg.text}
                </div>
        ...
        <div className="p-4 border-t border-[#2a2a2a]">
          <div className="flex gap-3">
            <input
              ...
              className="flex-1 bg-[#0d0d0d] border border-[#2a2a2a] rounded-xl px-4 py-3 text-[#f0f0f0] text-sm placeholder-[#555] focus:outline-none focus:border-[#22c55e]/50"
              onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); setMessage(""); } }}
            />
            <button
              onClick={() => setMessage("")}
              className="w-11 h-11 rounded-xl bg-[#22c55e] hover:bg-[#16a34a] flex items-center justify-center flex-shrink-0 transition-colors"
            >
